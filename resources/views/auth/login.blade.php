    ...
    <div class="w-full md:w-1/2 lg:w-3/5 flex items-center justify-center">
        <div class="w-full max-w-md p-8">
            <div class="bg-white rounded-2xl shadow-md p-8">
                ...
                <p class="text-sm text-slate-500 mt-2">Please enter your details.</p>
                ...
                <form method="POST" action="{{ route('login') }}" class="mt-6">
                    ...
                    <div class="flex items-center justify-between mt-4">
                        <label class="flex items-center text-sm">
                            <input type="checkbox" name="remember" id="remember" class="h-4 w-4 text-[#2F4156] focus:ring-[#567C8D] border-gray-300 rounded" />
                            <span class="ml-2 text-slate-600">Remember me</span>
                        </label>

                        <div>
                            @if (Route::has('password.request'))
                                <a class="text-sm text-slate-500 hover:underline" href="{{ route('password.request') }}">Forgot your password?</a>
                            @endif
                        </div>
                    </div>

                    <div class="mt-6">
                        <button type="submit" class="w-full bg-[#2F4156] text-white py-3 rounded-lg font-semibold hover:bg-[#1a2332] transition">Log in</button>
                    </div>
                </form>

                <p class="text-sm text-slate-500 text-center mt-6">Don't have an account? <a href="{{ route('register') }}" class="text-brand-dark font-semibold">Sign up</a></p>
            </div>
        </div>
    </div>
</div>
